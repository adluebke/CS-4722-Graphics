...
void render() {
	...
	glDrawArrays(GL_TRIANGLES, 0, 3); // Draw three vertices
...
#pragma region SHADER_STUFF
	...
	glBufferData(GL_ARRAY_BUFFER,
		3*7*sizeof(GLfloat), NULL, GL_STATIC_DRAW);

	glBufferSubData(GL_ARRAY_BUFFER,
		0, 3*3*sizeof(GLfloat), vertices);

	glBufferSubData(GL_ARRAY_BUFFER,
		3*3*sizeof(GLfloat), 3*4*sizeof(GLfloat), colors);
	...
	glVertexAttribPointer(attribLocID2, 4, 
		GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(3*3*sizeof(GLfloat)));
